...
class BdAction
{
    ...
    /**
     * Busca número de resultados
     * @param null $parameters
     * @return int
     * @throws Exception
     */
    public function getCount($parameters = null)
    {
        if ($parameters) {
            $this->mount($parameters);
        }
        ...
        $parameters = $this->generateBindValues();
        $where = $this->generateWhereBind($parameters);
        $sql = "SELECT COUNT(*) AS QTD FROM " . $this->getTabela();

        if (sizeof($parameters) > 0) {
            $sql = "SELECT COUNT(*) AS QTD FROM " . $this->getTabela() . " WHERE " . $where;
        }

        $result = $this->getFirst($this->getResult($this->execute($sql, $parameters)));
        return $result["QTD"];
    }
     ...
     * @throws Exception
     */
    public function getMax($campo = null, $parameters = null)
    {
        ...
        $parameters = $this->generateBindValues();
        $where = $this->generateWhereBind($parameters);
        $sql = "SELECT MAX($campo) AS QTD FROM " . $this->getTabela();

        if (sizeof($parameters) > 0) {
            $sql = "SELECT MAX($campo) AS QTD FROM " . $this->getTabela() . " WHERE " . $where;
        }
     ...
     * @throws Exception
     */
    public function findCustom($select = "*", $where = "", $distinct = false, $order = array(), $limit = null, $parameters = null, $skip = null)
    {
        try {
            if ($parameters) {
                $this->mount($parameters);
            ...
            return $this->getResult($this->execute($sql, $parameters));
    ...
    public function validate($validate, $seletors = array(), $params = null, $validaPrimaKey = false, $lancaErro = true)
    {
        try {
            ...
            foreach (Validate::GLOBAL_VALIDATE as $key => $value) {
